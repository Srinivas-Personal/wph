angular.module('rezTrip')
  ...
  .service('rt3Browser', ['$rootScope', '$q', 'rt3api', 'rt3Search','$location', function($rootScope, $q, rt3api, rt3Search ,$location) {
    function Browser() {
      this.loaded = false;
      this.roomsTonight=[];
      this.rooms = [];
    ...
    RoomDetails.prototype.fetchRoomDetails = function() {
      ...
      $q.when(rt3api.getAllAvailableRooms()).then(function(response) {
        $.each(response.rooms, function(key, value) {
          if(value.code.toLowerCase() == self.params.room_id.toLowerCase()) {
